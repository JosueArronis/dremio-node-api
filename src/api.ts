...
export function createFirstUser(
  ...
  return raw.makeRequest(
    ...
    null,
    null,
    JSON.stringify(canonicaliseUser(user, password)),
    null
  ); //Buffer.from(JSON.stringify(canonicaliseUser(user, password))).toString('base64'))
}
...
export async function createAPI(
  config: ClusterConfiguration,
  login: schema.LoginBodyArgs
): Promise<API> {
  const token =
    "_dremio" +
    (
      await raw.makeRequest(
        config,
        "post",
        null,
        raw.LOGIN,
